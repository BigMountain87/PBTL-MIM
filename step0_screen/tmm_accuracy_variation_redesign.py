...
# (struct_A_500_redesign.npz, unified 400-1800nm, jc materials, adaptive order).
# Changes vs tmm_accuracy_variation.py: grid loaded from data file; MATERIAL_MODEL='jc';
# RCWA data -> *_redesign.npz; reliable-mask filter; output _redesign tag; device auto;
# home paths (runs on home GPU alongside the pbtl_*_redesign runs).
HOME = '.'
...
print(f"Device: {device} | MATERIAL_MODEL={_mat.MATERIAL_MODEL}", flush=True)


def pearson_median_per_sample(a, b):
    """Median per-sample Pearson r over wavelengths. Matches the Table-I fidelity
    definition and the Structure-B/C noise replicas; replaces the earlier
    pooled-flatten Pearson so the operating-band fidelity metric is consistent
    across all noise tables (A, B, C)."""
    rs = []
