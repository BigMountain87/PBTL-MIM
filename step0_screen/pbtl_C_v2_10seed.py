...
# Note (added during 2026-05 audit): for Structure C the post-filter pool has
# only ~400 valid samples, leaving 300 candidates after the 50+50 test/val
# reservation above. The label 350 therefore exceeds the available pool, and
# `permutation(len(remaining))[:350]` silently returns the full 300 samples.
# The label is preserved here only to match historical output filenames; the
# corrected manuscript reports this row as effective n=300.
...
np.savez("results/pbtl_C_v2_10seed.npz", train_sizes=TRAIN_SIZES, seeds=SEEDS,
...
print("Results saved: pbtl_C_v2_10seed.npz", flush=True)
