#!/usr/bin/env python3
"""
TMM Pre-training Data Size Sensitivity Experiment -- REDESIGN variant (Supp S2).

Tests how the number of TMM pre-training samples (N_TMM) affects transfer-learning
performance (M_TL) on the corrected REDESIGN Structure A dataset. Used to support the
"saturation beyond N_TMM ~= 2000" claim.

Conditions:
  TMM sizes : [500, 1000, 2000, 5000]
  RCWA train: n=100 and n=350
  Seeds     : [42, 123, 777]
  Baseline  : M0 trained from scratch (no pre-training)

REDESIGN porting changes vs step0_screen/tmm_size_sensitivity.py
(mirrors pbtl_A_redesign.py):
  (1) MATERIAL_MODEL='jc' set BEFORE importing tmm modules (matches RCWA labels).
  (2) RCWA data loaded from data/raw/struct_A_500_redesign.npz (not struct_A_vis_500).
  (3) wavelength grid LOADED from the npz (unified 400-1800nm, 100pt; not 380-780).
  (4) sample filter uses the disclosed `reliable.all(axis=1)` mask.
  (5) <repo root> -> <repo root>.
  (6) device='cpu' (shared Mac GPU; small data), torch.set_num_threads(2).
  (7) output -> results/tmm_size_sensitivity_redesign.npz.
"""
...
# (1) jc materials BEFORE importing tmm modules
...
def train_model(model, dl_tr, dl_vl, epochs, lr):
    ...
    _xg, _a, _r = dl_tr[0]; _N = _xg.shape[0]
    for ep in range(epochs):
        model.train()
        _perm = torch.randperm(_N, device=_xg.device)
        for _i in range(0, _N, 131072):
            _ix = _perm[_i:_i+131072]; x, a, r = _xg[_ix], _a[_ix], _r[_ix]
            out = model(x)
...
# (3) wavelength grid from the REDESIGN data file (unified 400-1800nm, 100pt)
...
# ======================== Load RCWA data ========================
print("\n=== Loading RCWA data (REDESIGN) ===", flush=True)
...
def make_rcwa_dl(rows, bs=131072, shuffle=False):
...
for n_train in RCWA_TRAIN_SIZES:
    for seed in SEEDS:
        ...
        dl_tr = make_rcwa_dl(tr_rows, bs=131072, shuffle=True)
...
pretrain_times = {}
tmm_val_maes = {}
tl_results = {}  # tl_results[(n_tmm, n_rcwa, seed)] -> MAE

for n_tmm in TMM_SIZES:
    print(f"\n--- N_TMM = {n_tmm} ---", flush=True)

    # Generate TMM data (jc materials via MATERIAL_MODEL set at top)
    ...
    def make_tmm_dl(rows, bs=131072, shuffle=False):
...
# ======================== Summary ========================
print("\n" + "=" * 100, flush=True)
print("TMM PRE-TRAINING DATA SIZE SENSITIVITY (REDESIGN, Structure A)", flush=True)
...
hdr = (f"{'N_TMM':>7} | {'TMM gen(s)':>10} | {'PT time(s)':>10} | "
       f"{'TMM val MAE':>12} | {'RCWA n=100':>16} | {'RCWA n=350':>16}")
print(f"\n{hdr}", flush=True)
...
print(f"{'scratch':>7} | {'---':>10} | {'---':>10} | {'---':>12} | "
      f"{bl_100.mean():.3f}+/-{bl_100.std():.3f}{'':>3} | "
      f"{bl_350.mean():.3f}+/-{bl_350.std():.3f}{'':>3}", flush=True)
...
for n_tmm in TMM_SIZES:
    ...
    print(f"{n_tmm:>7} | {gt:>10.1f} | {pt:>10.0f} | "
          f"{tv:>11.3f}% | "
          f"{r100.mean():.3f}+/-{r100.std():.3f}{'':>3} | "
          f"{r350.mean():.3f}+/-{r350.std():.3f}{'':>3}", flush=True)
...
savepath = "results/tmm_size_sensitivity_redesign.npz"
