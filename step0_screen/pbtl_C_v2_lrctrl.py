...
device = torch.device("cpu")  # forced CPU: shared Mac GPU across parallel agents
...
def train_model(model, dl_tr, dl_vl, epochs, lr, has_phys=False, dual=True):
    ...
    for ep in range(epochs):
        model.train()
        for batch in dl_tr:
            if has_phys: x,ate,rte,atm,rtm,p = batch; out = model(x, p=p)
            else: x,ate,rte,atm,rtm = batch; out = model(x)
...
# ===== Step 2: SKIPPED (lr-control: M0_dual from scratch only, no pretrain) =====
...
N_REMAINING_C = len(remaining)  # 387 with reliable_TE&TM filter -> n_train=350 genuine

# --- Resumable checkpoint: persist each (n_train, seed) M0@3e-4 immediately so a
#     harness kill of this long CPU sweep loses at most one in-flight model. ---
import json
CKPT="results/pbtl_C_v2_lrctrl_ckpt.json"
done={}
if os.path.exists(CKPT):
    try:
        with open(CKPT) as f: done=json.load(f)
        print(f"[resume] loaded {len(done)} completed (size,seed) results from checkpoint", flush=True)
    except Exception as e:
        print(f"[resume] checkpoint unreadable ({e}); starting fresh", flush=True); done={}

for n_train in TRAIN_SIZES:
    for seed in SEEDS:
        key=f"{n_train}_{seed}"
        if key in done:
            print(f"--- n_train={n_train}, seed={seed} [cached {done[key]*100:.3f}%] ---", flush=True)
            continue
        ...
        # M0_dual from scratch at lr=3e-4 (MATCHED to M_TL) — lr-control
        set_seed(seed); m0=M0_dual(geo_dim).to(device)
        m0=train_model(m0,dl_tr_m0,dl_vl_m0,1000,3e-4,False)
        val=eval_model(m0,dl_te_m0,False)
        done[key]=float(val)
        tmp=CKPT+".tmp"
        with open(tmp,"w") as f: json.dump(done,f)
        os.replace(tmp,CKPT)
        print(f'  M0_lr3e4: {val*100:.3f}%', flush=True)

# Reassemble results dict in canonical (size x seed) order from the checkpoint.
results={sz:{"M0_lr3e4":[done[f"{sz}_{seed}"] for seed in SEEDS]} for sz in TRAIN_SIZES}

print("\n"+"="*70, flush=True)
print("lr-CONTROL: M_TL(3e-4) vs M0(3e-4) — Structure C", flush=True)
print("="*70, flush=True)
mtl=m0orig=None
try:
    sv=np.load("results/pbtl_C_v2_redesign_10seed.npz",allow_pickle=True)
...
np.savez("results/pbtl_C_v2_lrctrl.npz", train_sizes=TRAIN_SIZES, seeds=SEEDS,
         M0_lr3e4=[results[sz]["M0_lr3e4"] for sz in TRAIN_SIZES])
print("Saved: pbtl_C_v2_lrctrl.npz", flush=True)
