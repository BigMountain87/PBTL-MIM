#!/usr/bin/env python3
"""Data efficiency experiment: M0 vs M_phys for Structure B (Ring-Disk Fano)."""
import sys, os
import os
...
def compute_physics_features_B(params, wavelengths_nm, metal='Cr'):
    """Physics features for Structure B: Ring-Disk Fano MIM.
    Layers: Air | Patterned Cr (ring+disk) | SiO2 | Cr mirror | Glass
    ...
    feats = []
    # 1-2: Fabry-Perot cavity phase (SiO2 layer)
    ...
    # 3: Ring fill fraction = pi*(R_out^2 - R_in^2) / P^2
    ring_area = np.pi * (R_out**2 - R_in**2)
    feats.append(np.tile((ring_area / P**2)[:,None], (1,Nlam)))   # 3

    # 4: Disk fill fraction = pi*R_disk^2 / P^2
    disk_area = np.pi * R_disk**2
    feats.append(np.tile((disk_area / P**2)[:,None], (1,Nlam)))   # 4

    # 5: Total metal fill fraction
    total_metal = ring_area + disk_area
    feats.append(np.tile((total_metal / P**2)[:,None], (1,Nlam)))  # 5

    # 6-7: Sub-wavelength ratios
    feats.append(P[:,None] / wavelengths_nm[None,:])    # 6
    feats.append(R_out[:,None] / wavelengths_nm[None,:]) # 7

    # 8: Skin depth ratio
    feats.append(t_Cr[:,None] / skin_depth[None,:])      # 8

    # 9: Optical path length
    feats.append(n_sio2[None,:] * d_SiO2[:,None] / wavelengths_nm[None,:]) # 9

    # 10: cos(theta)
    feats.append(np.tile(np.cos(theta_rad[:,None]), (1,Nlam)))  # 10

    # 11: Ring gap ratio (Fano coupling strength) = (R_out - R_in) / R_out
    gap_ratio = (R_out - R_in) / (R_out + 1e-10)
    feats.append(np.tile(gap_ratio[:,None], (1,Nlam)))  # 11

    # 12: Disk-to-ring ratio (Fano asymmetry) = R_disk / R_in
    disk_ring = R_disk / (R_in + 1e-10)
    feats.append(np.tile(disk_ring[:,None], (1,Nlam)))  # 12
...
# ========= Load data =========
datapath = 'data/raw/struct_B_500.npz'
...
# Filter bad samples (negative A)
good = np.all(A_all >= -0.01, axis=1)
gi = np.where(good)[0]
N = len(gi)
params = params_all[gi]; A_arr = A_all[gi]; R_arr = R_all[gi]
# Clamp small negatives
A_arr = np.clip(A_arr, 0, 1)
...
params_norm = (params - BOUNDS_B[:,0]) / (BOUNDS_B[:,1] - BOUNDS_B[:,0])

phys = compute_physics_features_B(params.astype(np.float64), wavelengths, 'Cr')
n_phys = phys.shape[-1]
...
geo_dim = 1 + params.shape[1]  # wavelength + 8 params = 9
...
X_te=torch.tensor(X_geo[test_rows]).to(device); A_te=torch.tensor(A_flat[test_rows]).to(device)
R_te=torch.tensor(R_flat[test_rows]).to(device); P_te=torch.tensor(X_phys_n[test_rows]).to(device)
X_vl=torch.tensor(X_geo[val_rows]).to(device); A_vl=torch.tensor(A_flat[val_rows]).to(device)
R_vl=torch.tensor(R_flat[val_rows]).to(device); P_vl=torch.tensor(X_phys_n[val_rows]).to(device)

dl_te_m0 = DataLoader(TensorDataset(X_te,A_te,R_te), batch_size=2048)
dl_te_ph = DataLoader(TensorDataset(X_te,A_te,R_te,P_te), batch_size=2048)
...
for n_train in TRAIN_SIZES:
    ...
    for seed in SEEDS:
        ...
        X_tr=torch.tensor(X_geo[tr_rows]).to(device); A_tr=torch.tensor(A_flat[tr_rows]).to(device)
        R_tr=torch.tensor(R_flat[tr_rows]).to(device); P_tr=torch.tensor(X_phys_n[tr_rows]).to(device)
...
# ========= Summary =========
print('\n' + '='*70, flush=True)
print('DATA EFFICIENCY: M0 vs M_phys (Structure B, Ring-Disk Fano)', flush=True)
print('='*70, flush=True)
...
# Save results
np.savez('results/data_efficiency_B.npz',
