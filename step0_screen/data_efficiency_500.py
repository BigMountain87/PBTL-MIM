#!/usr/bin/env python3
"""
Data efficiency experiment with 500 samples: M0 vs M_phys at different training set sizes.
Uses 500-sample dataset: train sizes [50, 100, 200, 350], test=50 fixed, val=50 fixed.
"""
import sys, os
import os
...
print(f"Data: {N} good samples out of {len(params_all)}, {Nlam} wavelengths")
...
N_TEST = 50   # fixed test set
N_VAL = 50    # fixed val set
SEEDS = [42, 123, 777]
EPOCHS = 5000

# Fixed test+val set (last 100 samples in shuffled order)
rng_split = np.random.default_rng(42)
all_idx = rng_split.permutation(N)
test_idx = all_idx[-N_TEST:]
val_idx = all_idx[-(N_TEST+N_VAL):-N_TEST]
remaining = all_idx[:-(N_TEST+N_VAL)]
print(f"Test: {len(test_idx)}, Val: {len(val_idx)}, Pool: {len(remaining)}")
...
A_vl = torch.tensor(A_flat[val_rows]).to(device)
R_vl = torch.tensor(R_flat[val_rows]).to(device)
...
ds_vl_m0 = TensorDataset(X_vl, A_vl, R_vl)
ds_vl_ph = TensorDataset(X_vl, A_vl, R_vl, P_vl)
...
for n_train in TRAIN_SIZES:
    if n_train > len(remaining):
        print(f"Skipping n_train={n_train} (only {len(remaining)} available)")
        continue
...
# ========= Summary =========
print("\n" + "="*70)
print("DATA EFFICIENCY: M0 vs M_phys (Structure A, Visible, 500 samples)")
print("="*70)
print(f"Total good samples: {N}")
print(f"Test: {N_TEST}, Val: {N_VAL}, Pool: {len(remaining)}")
print(f"Seeds: {SEEDS}")
print()
