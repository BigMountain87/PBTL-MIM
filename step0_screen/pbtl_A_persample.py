...
def train_model(model, tr, vl, epochs, lr, has_phys=False, bs=512):
    """Chunk-over-GPU-resident-tensors training (DataLoader-free; avoids the dispatch
    hang seen on this box). tr/vl are dicts of GPU tensors {'x','a','r','p'}; equivalent
    to shuffled mini-batch SGD with the same bs/epochs/lr."""
    ...
    for ep in range(epochs):
        ...
        for s in range(0, N, bs):
            idx = perm[s:s+bs]
            out = model(tr['x'][idx], p=tr['p'][idx]) if has_phys else model(tr['x'][idx])
            loss = crit(out["A"], tr['a'][idx]) + crit(out["R"], tr['r'][idx])
        ...
        if (ep+1) % 100 == 0:
            ...
            with torch.no_grad():
                ...
                vm = (nn.functional.l1_loss(out["A"], vl['a']) +
                      nn.functional.l1_loss(out["R"], vl['r'])).item() / 2.0
                if vm < best_vl:
                    best_vl = vm
...
def eval_model(model, te, has_phys=False):
    """Evaluate model, return MAE on absorptance (full-pass on a GPU-resident dict)."""
    model.eval()
    with torch.no_grad():
        out = model(te['x'], p=te['p']) if has_phys else model(te['x'])
        return nn.functional.l1_loss(out["A"], te['a']).item()
...
def to_dl(rows, has_phys, bs=2048, shuffle=False):
    return {'x': torch.tensor(X_geo_tmm[rows]).to(device),
...
TRAIN_SIZES = [350]  # R2 figs: deployment size only (full epochs)
SEEDS = [42]  # R2 figs: single representative seed (full epochs)
...
for n_train in TRAIN_SIZES:
    ...
    for seed in SEEDS:
        ...
        m0 = train_model(m0, dl_tr_m0, dl_vl_m0, FINETUNE_EPOCHS, FINETUNE_LR, has_phys=False, bs=512)
        ...
        mp = train_model(mp, dl_tr_ph, dl_vl_ph, FINETUNE_EPOCHS, FINETUNE_LR, has_phys=True, bs=512)
        ...
        m_tl = train_model(m_tl, dl_tr_m0, dl_vl_m0, FINETUNE_EPOCHS, FINETUNE_LR_TL, has_phys=False, bs=512)
        ...
        m_tlp.load_state_dict(torch.load("results/pretrained_mphys_tmm.pt",
                                          map_location=device, weights_only=True))
        m_tlp = train_model(m_tlp, dl_tr_ph, dl_vl_ph, FINETUNE_EPOCHS, FINETUNE_LR_TL, has_phys=True, bs=512)
        mae = eval_model(m_tlp, dl_te_ph, has_phys=True)
        results[n_train]["M_TL+phys"].append(mae)
        print(f"  M_TL+phys: {mae*100:.3f}%", flush=True)

        # === R2 Fig S1/S2: per-sample test predictions (full-epoch, n=350, seed 42) ===
        if n_train == 350 and seed == 42:
            from src.simulation.tmm_struct_a import compute_tmm_batch as _tmmb
            def _persample(model, has_phys):
                model.eval()
                with torch.no_grad():
                    xg = torch.tensor(X_geo_rcwa[test_rows]).to(device)
                    if has_phys:
                        pp = torch.tensor(X_phys_rcwa_n[test_rows]).to(device)
                        o = model(xg, p=pp)
                    else:
                        o = model(xg)
                    return o["A"].cpu().numpy().reshape(len(test_idx), Nlam)
            A_pred_tlp = _persample(m_tlp, True)
            A_pred_m0 = _persample(m0, False)
            A_te = A_rcwa[test_idx]
            A_tmm_te = _tmmb(params_rcwa[test_idx], wavelengths_rcwa, "Cr")["A_tmm"]
            ps_mae_tlp = np.mean(np.abs(A_pred_tlp - A_te), axis=1)
            ps_mae_m0 = np.mean(np.abs(A_pred_m0 - A_te), axis=1)
            ps_r = np.array([np.corrcoef(A_tmm_te[i], A_te[i])[0, 1] for i in range(len(test_idx))])
            np.savez("results/persample_A_r2.npz",
                     ps_mae_tlp=ps_mae_tlp, ps_mae_m0=ps_mae_m0, ps_r=ps_r,
                     A_rcwa=A_te, A_tmm=A_tmm_te, A_pred_tlp=A_pred_tlp,
                     wl=wavelengths_rcwa, test_idx=test_idx)
            print(f"  [R2] per-sample saved: n={len(test_idx)} MAE_TLphys_med={np.median(ps_mae_tlp)*100:.2f}% r=[{ps_r.min():.2f},{ps_r.max():.2f}]", flush=True)
...
# Save
savepath = "results/pbtl_A_redesign_10seed.npz"
