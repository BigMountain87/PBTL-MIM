#!/usr/bin/env python3
"""PBTL 4-way comparison for Au/SiO2 Structure A on the CORRECTED J&C Au labels.

Port of step0_screen/pbtl_Au_5seed.py with two changes only:
  (1) MATERIAL_MODEL='jc' set BEFORE any tmm import (measured J&C Au), and the RCWA labels
      loaded from the re-simulated struct_A_Au_500_jc.npz (jc + adaptive + c64 + grazingfix)
  ...
  (2) The torch DataLoader training loop is REPLACED by chunk-over-GPU-resident tensors
      ...
      results stay directly comparable to the prior Au-A numbers (modulo the materials fix).
...
# Redesign reliability filter: reliable at ALL wavelengths, then clip A/R to [0,1].
...
# ========= Summary =========
print("\n" + "="*80, flush=True)
print("Au PBTL 4-WAY COMPARISON: Structure A (jc-corrected)", flush=True)
print("="*80, flush=True)
